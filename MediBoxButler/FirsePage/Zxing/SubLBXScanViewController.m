...
//    self.navigationController.navigationBar.barStyle = UIBarStyleBlack;
    ...
    [self.navigationController.navigationBar setBarTintColor:_bottomItemsView.backgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.6]];
...
- (void)drawTitle
{
    if (!_topTitle)
    {
        ...
        _topTitle.center = CGPointMake(CGRectGetWidth(self.view.frame)/2, 55);
...
- (void)drawBottomItems
{
    ...
    _btnFlash.center = CGPointMake(CGRectGetWidth(_bottomItemsView.frame)/2 + (CGRectGetWidth(_bottomItemsView.frame)/4), CGRectGetHeight(_bottomItemsView.frame)/2);
     [_btnFlash setImage:[UIImage imageNamed:@"CodeScan.bundle/qrcode_scan_btn_flash_nor"] forState:UIControlStateNormal];
    [_btnFlash addTarget:self action:@selector(openOrCloseFlash) forControlEvents:UIControlEventTouchUpInside];
    
    self.btnPhoto = [[UIButton alloc]init];
    _btnPhoto.bounds = _btnFlash.bounds;
    _btnPhoto.center = CGPointMake((CGRectGetWidth(_bottomItemsView.frame)/2) - (CGRectGetWidth(_bottomItemsView.frame)/4), CGRectGetHeight(_bottomItemsView.frame)/2);
